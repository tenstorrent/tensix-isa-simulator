...
#define PRINT_TEXT rvtt->print_text

namespace sfpi_test {

using namespace ronin::iss::whb0::sfpi;

void copy_result_to_dreg0(int addr) {
...
void set_expected_result(int addr, float sentinel, int expected, vInt v) {
    // Poor man's equals
...
vInt test_interleaved_scalar_vector_cond(bool scalar_bool, vFloat vec, float a, float b) {
    if (scalar_bool) {
        return vec == a;
    } else {
        return vec == b;
...
vType reduce_bool4(vType a, vType b, vType c, vType d, int reference) {
    vType result1 = 0;
...
PRINT_TEXT("v_if(dst_reg[0] == 0.0F)");
    v_if(dst_reg[0] == 0.0F) {
        // 1 load
        dst_reg[3] = 10.0F;
PRINT_TEXT("v_else");
...
PRINT_TEXT("v_if(dst_reg[0] == 2.0F)");
    v_if(dst_reg[0] == 2.0F) {
        // 1 load
        vFloat a = 30.0F;
        dst_reg[3] = a;
...
PRINT_TEXT("v_if(dst_reg[0] == 3.0F)");
    v_if(dst_reg[0] == 3.0F) {
        // 2 loads
        dst_reg[3] = 1.005f;
    }
...
PRINT_TEXT("v_if(dst_reg[0] == 4.0F)");
    v_if(dst_reg[0] == 4.0F) {
        // 2 loads
        vFloat a = 1.005f;
        dst_reg[3] = a;
...
PRINT_TEXT("v_if(dst_reg[0] == 5.0F)");
    v_if(dst_reg[0] == 5.0F) {
...
PRINT_TEXT("v_if(dst_reg[0] == 6.0F)");
    v_if(dst_reg[0] == 6.0F) {
...
PRINT_TEXT("v_if(dst_reg[0] == 7.0F)");
    v_if(dst_reg[0] == 7.0F) {
...
PRINT_TEXT("v_if(dst_reg[0] == 8.0F)");
    v_if(dst_reg[0] == 8.0F) {
...
PRINT_TEXT("v_if(dst_reg[0] == 9.0F)");
    v_if(dst_reg[0] == 9.0F) {
        // This will be an int w/ 2 loads
...
PRINT_TEXT("v_if(dst_reg[0] == 10.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 11.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 12.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 13.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 14.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 15.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 16.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 17.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 18.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 19.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 20.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 21.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 22.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 23.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 24.0F)");
...
PRINT_TEXT("v_if(dst_reg[0] == 25.0F)");
...
PRINT_TEXT("v_if(v >= 18.0F && v < 23.0F)");
...
PRINT_TEXT("v_if(!(v >= 25.0F) && !(v < 24.0F))");
...
PRINT_TEXT("v_endif");
    ...
    // Test ! on Boolean
PRINT_TEXT("v_if(!((v < 25.0F) || (v >= 26.0F)))");
...
PRINT_TEXT("v_if((v >= 26.0F) && (v < 29.0F))");
...
PRINT_TEXT("v_if ((v >= 28.0F) && (v == 29.0F || v == 30.0F || v == 31.0F))");
...
PRINT_TEXT("v_if (dst_reg[0] == 26.0F)");
...
PRINT_TEXT("v_if (dst_reg[0] >= 28.0F)");
...
PRINT_TEXT("v_if (dst_reg[0] == 31.0F)");
...
PRINT_TEXT("---- test7");
    ...
    dst_reg[7] = -dst_reg[0];
PRINT_TEXT("v_if(dst_reg[0] == 1.0F)");
    v_if(dst_reg[0] == 1.0F) {
        vFloat tmp = 124.05F;
...
PRINT_TEXT("v_endif");
    ...
    vFloat v = dst_reg[0];
PRINT_TEXT("v_if ((v >= 12.0f && v < 14.0f) || (v >= 15.0f && v < 17.0f))");
...
PRINT_TEXT("v_if(((v >= 17.0f && v < 18.0f) || (v >= 19.0f && v < 20.0f)) || ...)");
...
PRINT_TEXT("v_if (v >= 25.0f && v < 29.0f)");
...
PRINT_TEXT("v_if (v >= 29.0f && v < 32.0f)");
...
PRINT_TEXT("v_if (test_interleaved_scalar_vector_cond(true, dst_reg[0], 14.0F, 15.0F))");
...
PRINT_TEXT("v_endblock");
    v_endblock;
    dst_reg[8] = tmp;

    // <= and > are compound statements in the compiler, <= uses a compc
    // and things get flipped around when joined by ||
PRINT_TEXT("v_if (dst_reg[0] >= 20.0f && dst_reg[0] < 23.0f)");
...
PRINT_TEXT("v_if (dst_reg[0] >= 23.0f && dst_reg[0] < 26.0f)");
...
PRINT_TEXT("v_if (dst_reg[0] >= 26.0f && dst_reg[0] < 29.0f)");
...
PRINT_TEXT("v_if(dst_reg[0] == 29.0F)");
    v_if(dst_reg[0] == 29.0F) {
        vInt a = 0xA5A5;
        vInt b = 0xFF00;
        vInt c = a ^ b;
...
PRINT_TEXT("v_if(dst_reg[0] == 30.0F)");
...
PRINT_TEXT("v_if (dst_reg[0] >= 15.0F && dst_reg[0] < 19.0)");
...
PRINT_TEXT("v_if (dst_reg[0] >= 19.0F && dst_reg[0] < 23.0)");
...
PRINT_TEXT("v_if ((dst_reg[0] == 23.0 || dst_reg[0] == 24.0 || ...)");
...
#endif
PRINT_TEXT("TTI_SFPLOADI(0, SFPLOADI_MOD0_USHORT, 0xFF20);");
PRINT_TEXT("TTI_SFPLOADI(1, SFPLOADI_MOD0_USHORT, 0x2010);");
        vUInt l0b, l1b;
...
PRINT_TEXT("v_if(dst_reg[0] == 16.0F)");
    v_if(dst_reg[0] == 16.0F) {
        ...
        dst_reg[12] = a;
        rvtt->sfpnop(); // XXXXXX remove me when compiler is fixed
        a = dst_reg[imm - 23];
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 0.0F || dst_reg[0] == 1.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 2.0F || dst_reg[0] == 3.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 4.0F || dst_reg[0] == 5.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 6.0F || dst_reg[0] == 7.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 8.0F || dst_reg[0] == 9.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 10.0F || dst_reg[0] == 11.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 12.0F || dst_reg[0] == 13.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 14.0F || dst_reg[0] == 15.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 16.0F || dst_reg[0] == 17.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 18.0F || dst_reg[0] == 19.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 20.0F || dst_reg[0] == 21.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 22.0F || dst_reg[0] == 23.0F)");
...
PRINT_TEXT("v_endif");
        v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 24.0F || dst_reg[0] == 25.0F)");
...
PRINT_TEXT("v_if (dst_reg[0] == 100.0f)");
...
PRINT_TEXT("v_if (condition == 0.0F)");
        v_if (condition == 0.0F) {
            c = a - b;
        }
PRINT_TEXT("v_endif");
...
PRINT_TEXT("v_if (vConst0p8373 == dst_reg[0])");
...
PRINT_TEXT("v_if (dr == 26.0F || dr == 27.0F)");
...
PRINT_TEXT("[%d] v_if (dst_reg[0] == 28.0F)", i);
            v_if (dst_reg[0] == 28.0F) {
                switch (i) {
                case 0:
                    b = 2.0f;
...
PRINT_TEXT("v_if (dst_reg[0] == 28.0F || dst_reg[0] == 29.0F)");
...
PRINT_TEXT("v_if (dst_reg[0] == 10.0F)");
    v_if (dst_reg[0] == 10.0F) {
        dst_reg[16] = float_to_fp16a(1.32332f);
    }
PRINT_TEXT("v_endif");
    v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 11.0F)");
    v_if (dst_reg[0] == 11.0F) {
        dst_reg[16] = float_to_fp16b(1.32332f);
    }
PRINT_TEXT("v_endif");
    v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 12.0F)");
    v_if (dst_reg[0] == 12.0F) {
        set_expected_result(16, 48.0f, 24, float_to_uint8(23.3f));
    }
PRINT_TEXT("v_endif");
    v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 13.0F)");
    v_if (dst_reg[0] == 13.0F) {
        set_expected_result(16, 64.0f, 24, float_to_int8(23.3f));
    }
PRINT_TEXT("v_endif");
    v_endif;
PRINT_TEXT("v_if (dst_reg[0] == 14.0F)");
...
PRINT_TEXT("v_if (dst_reg[0] == 5.0F || dst_reg[0] == 6.0F)");
...
void calculate_sfpi(
    ...
    rvtt->set_hw(&hw);
    rvtt->reset();
    if (operation == SfpiTestType::test1) {
        test1();
    } else if (operation == SfpiTestType::test2) {
        test2();
    } else if (operation == SfpiTestType::test3) {
        test3();
    } else if (operation == SfpiTestType::test4) {
        test4();
    } else if (operation == SfpiTestType::test5) {
        test5();
    } else if (operation == SfpiTestType::test6) {
        test6();
    } else if (operation == SfpiTestType::test7) {
        test7();
    } else if (operation == SfpiTestType::test8) {
        test8();
    } else if (operation == SfpiTestType::test9) {
        test9();
    } else if (operation == SfpiTestType::test10) {
        test10();
    } else if (operation == SfpiTestType::test11) {
        test11();
    } else if (operation == SfpiTestType::test12) {
        test12(param0);
    } else if (operation == SfpiTestType::test13) {
        test13(param0);
    } else if (operation == SfpiTestType::test14) {
        test14(param0);
    } else if (operation == SfpiTestType::test15) {
        test15();
    } else if (operation == SfpiTestType::test16) {
        test16();
    } else if (operation == SfpiTestType::test17) {
        test17();
    }
    rvtt->compile();
    rvtt->set_hw(nullptr);
