...
inline void sfpu_load_imm32(uint32_t dest, uint32_t val) {
    // insmod == A will write the lower bits, and not affect the upper bits
    TT_SFPLOADI(dest, 0xA, (val & 0xFFFF));  
    // insmod == 8 will write the upper bits, and not affect the lower bits
    TT_SFPLOADI(dest, 0x8, (val>>16) & 0xFFFF);
}

inline void sfpu_load_config32(uint32_t dest, uint32_t upper16, uint32_t lower16) {
    // registers 11 through 14 are programmable "constants" which are shared across all 4 rows
    // They are updated only through the CONFIG path, which uses LREG[0] first and then copies it
    // to the desired register location
    // insmod == A will write the lower bits, and not affect the upper bits
    TTI_SFPLOADI(0, 0xA, lower16);
    // insmod == 8 will write the upper bits, and not affect the lower bits
    TTI_SFPLOADI(0, 0x8, upper16);
    TTI_SFPCONFIG(0, dest, 0);
}

sfpi_inline vInt sfpu_is_fp16_zero(const vFloat &v, uint32_t exponent_size_8) {
    if (exponent_size_8) {
        // fp16b
        return v == 0.0F;
    } else {
...
template <bool APPROXIMATION_MODE>
inline void sfpu_init(SfpuType operation, uint32_t param0 = 0) {
...
inline void calculate_rsqrt() {
    ...
    // ACHTUNG: For argument values < 1/3 Newton method converges to negative root
    //     This can be fixed by computing abs(result) after loop completion
...
// TODO(orig): Implement using bitwise comparision
template <bool APPROXIMATION_MODE, int ITERATIONS>
...
template <bool APPROXIMATION_MODE, int ITERATIONS>
inline void calculate_hardtanh(uint32_t param0, uint32_t param1, uint32_t param2) {
    begin_sfpi();
...
template <bool APPROXIMATION_MODE, int ITERATIONS>
inline void calculate_dropout(uint32_t prob, uint32_t scale) {
    begin_sfpi();
...
template <bool APPROXIMATION_MODE,int ITERATIONS>
inline void calculate_power_iterative(uint32_t exponent) {
    begin_sfpi();
...
sfpi_inline void calculate_log_body(uint32_t log_base_scale_factor) {
    ...
    vFloat series_result = x * (x * (x * a + b) + 2.0871f) + -1.4753f;
...
inline void calculate_log(uint32_t log_base_scale_factor) {
    begin_sfpi();
...
inline void calculate_comp(uint32_t exponent_size_8) {
    begin_sfpi();
...
template <bool APPROXIMATION_MODE, int ITERATIONS>
inline void calculate_clamp(uint32_t param0, uint32_t param1, uint32_t param2) {
    begin_sfpi();
...
sfpi_inline vFloat sfpu_asine_maclaurin_series(vFloat val) {
    ...
    // (5/112) * x^7
    tmp = tmp * val_square;
    output += 0.044642857f * tmp;

    // (35/1152) *x^9
    tmp = tmp * val_square;
    output += 0.03038194f * tmp;

    // (63/2816) * x^11
    tmp = tmp * val_square;
    output += 0.02237216f * tmp;
...
template <bool APPROXIMATION_MODE, int ITERATIONS>
inline void calculate_heaviside(uint32_t value) {
    begin_sfpi();
...
inline void calculate_sfpu(
    ...
    if constexpr (operation == SfpuType::exp_with_base) {
        ...
        // The algorithm uses Newton's method based on no. of iteration
        // better approximation can be calculated
        if (param0) {
