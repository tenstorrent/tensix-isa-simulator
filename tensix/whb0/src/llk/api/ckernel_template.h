...
// for unused fields in ckernel_unpack_template
constexpr Op NULL_OP = make_op(OpType(0));
...
public:
    ...
    ckernel_template(
        ...
        const Op &loop_op1);
public:
    void set_loop_op0(const Op &loop_op);
    void set_loop_op1(const Op &loop_op);
    void set_end_ops(const Op &end_op0, const Op &end_op1);
    void set_end_op(const Op &end_op0);
    void set_start_op(const Op &start_op0);
    void set_last_inner_loop_instr(const Op &op);
    void set_last_outer_loop_instr(const Op &op);
