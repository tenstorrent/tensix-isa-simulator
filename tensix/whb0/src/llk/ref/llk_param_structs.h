...
constexpr uint32_t default_tile_dims[2] = {32, 32};
...
struct llk_unpack_reduce_params_t {
    uint32_t unpA_operand;
    // uint32_t unpB_operand;  // TODO: Should be removed when llk hw args are cleaned up
};

struct llk_unpack_tilize_params_t {
    uint32_t unpA_operand;
    uint32_t unpA_block_c_dim;
};
...
struct llk_math_eltwise_unary_params_t {
    int32_t sfpu_params[6];  // TODO: Fix how we assign this from hlkc
    int32_t unused;
};
...
struct llk_relu_config_t {
    // 0 ? no relu, 1 ? val<0=>val=0, 2 ? val<threshold=>val=0, 3 - val>threshold=>val=threshold
    uint32_t ApplyRelu : 16;  
    uint32_t Threshold : 16;  // fp16
};
