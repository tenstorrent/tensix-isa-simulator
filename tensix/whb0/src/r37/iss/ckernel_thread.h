...
#define WAIT(cond) wait([&]() -> bool { return (cond); })
...
public:
    ...
    DataFormat get_cb_src_format(uint32_t cb_id) {
        assert(cb_id < 32);
        return DataFormat(m_cb_src_format[cb_id]);
    }
    DataFormat get_cb_dst_format(uint32_t cb_id) {
        assert(cb_id < 32);
        return DataFormat(m_cb_dst_format[cb_id]);
    }
    void write_globals();
private:
    CBInterface m_cb_interface[32];
    uint32_t m_cb_src_format[32];
    uint32_t m_cb_dst_format[32];
};
...
extern uint32_t *l1_buffer;

extern CBInterface *cb_interface;
extern const uint32_t *unpack_src_format;
extern const uint32_t *unpack_dst_format;
extern const uint32_t *pack_src_format;
extern const uint32_t *pack_dst_format;
